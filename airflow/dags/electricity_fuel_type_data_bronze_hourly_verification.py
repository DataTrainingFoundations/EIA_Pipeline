...
with DAG(
    dag_id="electricity_fuel_type_data_bronze_hourly_verification",
    ...
    tags=["eia", "bronze", "verification", "electricity_fuel_type_data"],
) as dag:
    stage_table = "ops.bronze_hourly_coverage_stage_fuel_{{ ts_nodash | lower }}"
    ...
    validate_stage_rows = build_validate_rows_task(
        ...
        stage_table,
        description="electricity_fuel_type_data bronze hourly coverage stage rows",
    ...
    validate_rows = build_validate_rows_task(
        ...
        params=["electricity_fuel_type_data"],
        description="electricity_fuel_type_data bronze hourly coverage rows",
    ...
    validate_distinct_hours = build_validate_distinct_task(
        ...
        where_clause="dataset_id = %s",
        params=["electricity_fuel_type_data"],
        description="electricity_fuel_type_data bronze hourly coverage distinct hours",
        allow_empty_result=True,
    )
    validate_non_negative_counts = build_validate_bounds_task(
        ...
        where_clause="dataset_id = %s",
        params=["electricity_fuel_type_data"],
        description="electricity_fuel_type_data bronze hourly observed row counts",
        allow_empty_result=True,
    )
