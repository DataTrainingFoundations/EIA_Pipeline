...
- **Focus trends** — hourly generation mix for one selected BA over the time window.
- **Map** — approximate geographic concentration of fossil vs renewable generation.
""")

# fact_hourly is the combined table — check generation rows exist
...
if not daily_df.empty:
    st.plotly_chart(
        px.area(
            daily_df,
            x="report_date", y="total_gwh", color="fuel_code",
...
if not fossil_rank.empty:
    rank_col1.plotly_chart(
        px.bar(
            ...
            x="fossil_pct", y="ba_code", orientation="h",
...
if not renewable_rank.empty:
    rank_col2.plotly_chart(
        px.bar(
            ...
            x="renewable_pct", y="ba_code", orientation="h",
...
if not focus_df.empty:
    st.plotly_chart(
        px.area(
            focus_df,
            x="period_display", y="generation_gwh", color="fuel_code",
            color_discrete_map=FUEL_COLORS,
            labels={
    ...
    share_df = (
        ...
        .apply(lambda g: pd.Series({
            "renewable_pct": g[g["fuel_code"].isin(["WND","SUN","WAT"])]["generation_gwh"].sum()
                             / g["generation_gwh"].sum() * 100
                             if g["generation_gwh"].sum() > 0 else None,
    ...
    share_df["metric"] = share_df["metric"].replace(
        {"renewable_pct": "Renewable %", "fossil_pct": "Fossil %"}
    )
    st.plotly_chart(
        px.line(
            share_df,
            x="period_display", y="pct", color="metric",
    ...
    if not focus_df.empty:
        st.download_button(
            ...
            data=focus_df[[
                ...
                "fuel_code", "fuel_name", "generation_gwh",
