...
with st.sidebar:
    st.image(
        "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8e/Electricity_symbol.svg/120px-Electricity_symbol.svg.png",
        width=40,
    )
...
with st.spinner("Loading grid operations data…"):
    ...
    snapshot_df = load_latest_grid_operations_snapshot(
        start_ts, end_ts, filtered_respondents
    )
    latest_alerts_df = load_latest_grid_alerts(start_ts, end_ts, filtered_respondents)
    alerts_history_df = load_grid_operations_alerts(
        start_ts, end_ts, filtered_respondents
    )
...
snapshot_df["period_display"] = convert_timestamp_series(
    snapshot_df["period"], display_tz
)
...
if not latest_alerts_df.empty:
    ...
    latest_alerts_df["period_display"] = convert_timestamp_series(
        latest_alerts_df["period"], display_tz
    )
    latest_alerts_df = coerce_numeric(
        latest_alerts_df, ["metric_value", "threshold_value"]
    )

if not alerts_history_df.empty:
    alerts_history_df["period"] = pd.to_datetime(alerts_history_df["period"], utc=True)
    alerts_history_df["period_display"] = convert_timestamp_series(
        alerts_history_df["period"], display_tz
    )
...
k2.metric(
    ...
    f"{n_in_alert:,}",
    delta=None,
    help="Respondents with ≥1 active alert in the latest snapshot",
)
k3.metric(
    "Critical priority",
    f"{n_critical:,}",
...
k5.metric(
    "Lowest coverage ratio",
    lowest_coverage_str,
    help="Minimum coverage_ratio across all respondents in the latest snapshot",
)
...
st.caption(
    "Ranked Critical → Elevated → Stable. Critical rows require immediate triage."
)
...
st.caption(
    "Latest-snapshot rankings. Use these to prioritise which respondent to focus on."
)
...
# Coverage ratio bar
coverage_rank = (
    snapshot_df.dropna(subset=["coverage_ratio"]).sort_values("coverage_ratio").head(10)
)
if coverage_rank.empty:
    ...
    fig_cov.add_vline(
        x=0.8, line_dash="dash", line_color="#E24B4A", annotation_text="0.8 threshold"
    )
...
if demand_melt.empty:
    ...
    fig_dem = px.line(
        ...
        color="series",
        color_discrete_map={
            "Actual demand": "#185FA5",
            "Day-ahead forecast": "#EF9F27",
        },
...
if ramp_df.empty:
    ...
    fig_ramp.add_hline(
        y=2.5, line_dash="dot", line_color="#E24B4A", annotation_text="+2.5σ", yref="y2"
    )
    fig_ramp.add_hline(
        y=-2.5,
        line_dash="dot",
...
if not filtered_alerts.empty:
    if sel_severity != "All":
        filtered_alerts = filtered_alerts[
            filtered_alerts["severity"].str.lower() == sel_severity.lower()
        ]
    if sel_type != "All":
...
with st.expander("Supporting analysis", expanded=False):
    ...
    map_df["lat"] = map_df["respondent"].map(
        lambda c: RESPONDENT_GEO.get(c, {}).get("lat")
    )
    map_df["lon"] = map_df["respondent"].map(
        lambda c: RESPONDENT_GEO.get(c, {}).get("lon")
    )
    ...
    if map_df.empty:
        ...
        sup_r.caption(
            "⚠️ Approximate respondent coordinates only — not primary operating evidence."
        )
