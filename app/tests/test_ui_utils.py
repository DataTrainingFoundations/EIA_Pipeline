...
def test_timestamp_conversion_numeric_helpers_and_priority_sorting() -> None:
    ...
    formatted = ui_utils.format_timestamp(pd.Timestamp("2026-03-10T12:00:00Z"), "America/Toronto")
    ...
    df = pd.DataFrame(
        {
            ...
            "metric": ["1.2", "bad", "3"],
            "timestamp": [pd.Timestamp("2026-03-10T10:00:00Z"), None, pd.Timestamp("2026-03-10T09:00:00Z")],
        }
    )
    ...
    assert ui_utils.latest_non_null_value(df, "timestamp", ["timestamp"]) == pd.Timestamp("2026-03-10T10:00:00Z")
    assert ui_utils.latest_non_null_value(pd.DataFrame(columns=["timestamp"]), "timestamp", ["timestamp"]) is None
