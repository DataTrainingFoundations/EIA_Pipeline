...
def fetch_page(
    ...
    for attempt in range(1, settings.max_retries + 1):
        try:
            logger.info(
                "Requesting EIA page route=%s start=%s end=%s offset=%s attempt=%s",
                route,
                start,
                end,
                offset,
            ...
            logger.info(
                "Received EIA page route=%s start=%s end=%s offset=%s rows=%s",
                route,
                start,
                end,
                offset,
            ...
            if attempt == settings.max_retries:
                raise
            logger.warning(
                "Retrying EIA request route=%s start=%s end=%s offset=%s after attempt=%s",
                route,
                start,
                end,
                offset,
...
def fetch_all_pages(settings: EiaSettings, dataset: dict, *, start: str, end: str) -> list[dict]:
    ...
    while True:
        ...
        if not page:
            logger.info(
                "Finished pagination dataset=%s route=%s start=%s end=%s total_rows=%s",
                dataset["id"],
                dataset["eia_route"],
                start,
                end,
        ...
        if len(page) < page_size:
            logger.info(
                "Final page dataset=%s route=%s start=%s end=%s page_rows=%s total_rows=%s",
                dataset["id"],
                dataset["eia_route"],
                start,
                end,
        ...
        offset += page_size
        logger.info(
            "Continuing pagination dataset=%s route=%s start=%s end=%s next_offset=%s total_rows=%s",
            dataset["id"],
            dataset["eia_route"],
            start,
            end,
