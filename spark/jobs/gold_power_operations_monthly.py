...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build curated Gold monthly electric power operations fact data."
    )
    parser.add_argument(
        "--silver-input-path", default="s3a://silver/electric_power_operational_data"
    )
    parser.add_argument(
        "--gold-output-path",
...
def build_power_operations_monthly_fact(power_df: DataFrame) -> DataFrame:
    ...
    ).orderBy(F.col("loaded_at").desc(), F.col("event_id").desc())
    gold_df = (
        ...
        .withColumn(
            "generation_mwh",
            F.when(
                F.col("generation_thousand_mwh").isNotNull(),
                F.greatest(F.col("generation_thousand_mwh"), F.lit(0.0))
                * F.lit(1000.0),
            ),
        )
        .withColumn(
            ...
            F.when(
                ...
                F.greatest(F.col("consumption_for_eg_thousand_units"), F.lit(0.0)),
            ),
        )
        .withColumn(
            "ash_content_pct",
            F.when(
                F.col("ash_content_pct").isNotNull(),
                F.greatest(F.col("ash_content_pct"), F.lit(0.0)),
            ),
        )
        .withColumn(
...
def main() -> None:
    ...
    if not has_partitioned_parquet_input(spark, args.silver_input_path):
        logger.info(
            "Skipping monthly power Gold build because no Silver input is available input_path=%s start=%s end=%s",
            args.silver_input_path,
            args.start,
            args.end,
        )
        return
    ...
    write_partitioned_parquet(
        gold_df, args.gold_output_path, partition_column="event_date"
    )
    log_job_complete(
        logger,
        "gold_power_operations_monthly",
