...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build grid operations hourly serving tables."
    )
    ...
    parser.add_argument(
        "--ops-stage-table", default="platinum.grid_operations_hourly_stage"
    )
    parser.add_argument(
        "--alerts-stage-table", default="platinum.grid_operations_alert_hourly_stage"
    )
...
def build_grid_operations_status(
    region_df: DataFrame, fuel_df: DataFrame
    ...
    status_df = (
        base_df.withColumn(
            "demand_ramp_mwh",
            F.col("actual_demand_mwh") - F.lag("actual_demand_mwh").over(order_window),
        )
        .withColumn(
            "generation_gap_mwh",
            F.when(
                F.col("total_generation_mwh").isNotNull(),
                F.col("total_generation_mwh") - F.col("actual_demand_mwh"),
            ),
        )
        .withColumn(
            "coverage_ratio",
        ...
        .withColumn(
            "forecast_error_avg", F.avg("forecast_error_mwh").over(stats_window)
        )
        .withColumn(
            "forecast_error_std",
            F.coalesce(
                F.stddev_pop("forecast_error_mwh").over(stats_window), F.lit(0.0)
            ),
        )
        .withColumn("demand_ramp_avg", F.avg("demand_ramp_mwh").over(stats_window))
        .withColumn(
            "demand_ramp_std",
            F.coalesce(F.stddev_pop("demand_ramp_mwh").over(stats_window), F.lit(0.0)),
        )
        .withColumn("demand_avg", F.avg("actual_demand_mwh").over(stats_window))
        .withColumn(
            "demand_std",
            F.coalesce(
                F.stddev_pop("actual_demand_mwh").over(stats_window), F.lit(0.0)
            ),
        )
        .withColumn(
            "forecast_error_zscore",
            F.when(
                F.col("forecast_error_std") > 0,
                (F.col("forecast_error_mwh") - F.col("forecast_error_avg"))
                / F.col("forecast_error_std"),
            ).otherwise(F.lit(0.0)),
        )
        .withColumn(
            "demand_ramp_zscore",
            F.when(
                F.col("demand_ramp_std") > 0,
                (F.col("demand_ramp_mwh") - F.col("demand_ramp_avg"))
                / F.col("demand_ramp_std"),
            ).otherwise(F.lit(0.0)),
        )
        .withColumn(
            "demand_zscore",
            F.when(
                F.col("demand_std") > 0,
                (F.col("actual_demand_mwh") - F.col("demand_avg"))
                / F.col("demand_std"),
            ).otherwise(F.lit(0.0)),
        )
        .drop(
        ...
        .withColumn(
            "alert_count",
            F.when(F.abs(F.col("forecast_error_zscore")) >= 2.5, F.lit(1)).otherwise(
                F.lit(0)
            )
            + F.when(F.abs(F.col("demand_ramp_zscore")) >= 2.5, F.lit(1)).otherwise(
                F.lit(0)
            )
            + F.when(F.col("coverage_ratio") < 0.9, F.lit(1)).otherwise(F.lit(0))
            + F.when(F.abs(F.col("demand_zscore")) >= 2.5, F.lit(1)).otherwise(
                F.lit(0)
            ),
        )
    ...
    assert_no_nulls(
        status_df,
        ["period", "respondent", "respondent_name", "actual_demand_mwh"],
        "platinum.grid_operations_hourly_stage",
    )
    assert_unique_keys(
        status_df, ["period", "respondent"], "platinum.grid_operations_hourly_stage"
    )
    assert_non_negative(
        status_df,
        [
    ...
    assert_value_bounds(
        ...
        min_value=0.0,
        max_value=100.0,
        label="platinum.grid_operations_hourly_stage",
    )
    assert_value_bounds(
        status_df,
        "fossil_share_pct",
    ...
    forecast_alerts = status_df.filter(
        F.abs(F.col("forecast_error_zscore")) >= 2.5
    ).select(
        "period",
        "respondent",
        "respondent_name",
        F.lit("forecast_error").alias("alert_type"),
        F.when(F.abs(F.col("forecast_error_zscore")) >= 3.5, F.lit("high"))
        .otherwise(F.lit("medium"))
        .alias("severity"),
        F.col("forecast_error_zscore").alias("metric_value"),
        F.lit(2.5).alias("threshold_value"),
        F.concat(
            F.lit("Forecast error z-score reached "),
            F.round(F.col("forecast_error_zscore"), 2),
        ).alias("message"),
    )

    ramp_alerts = status_df.filter(F.abs(F.col("demand_ramp_zscore")) >= 2.5).select(
        "period",
        "respondent",
        "respondent_name",
        F.lit("ramp_risk").alias("alert_type"),
        F.when(F.abs(F.col("demand_ramp_zscore")) >= 3.5, F.lit("high"))
        .otherwise(F.lit("medium"))
        .alias("severity"),
        F.col("demand_ramp_zscore").alias("metric_value"),
        F.lit(2.5).alias("threshold_value"),
        F.concat(
            F.lit("Demand ramp z-score reached "),
            F.round(F.col("demand_ramp_zscore"), 2),
        ).alias("message"),
    )

    coverage_alerts = status_df.filter(F.col("coverage_ratio") < 0.9).select(
        "period",
        "respondent",
        "respondent_name",
        F.lit("fuel_mix_support_gap").alias("alert_type"),
        F.when(F.col("coverage_ratio") < 0.8, F.lit("high"))
        .otherwise(F.lit("medium"))
        .alias("severity"),
        F.col("coverage_ratio").alias("metric_value"),
        F.lit(0.9).alias("threshold_value"),
        F.concat(
            F.lit("Generation coverage ratio dropped to "),
            F.round(F.col("coverage_ratio"), 2),
        ).alias("message"),
    )

    demand_alerts = status_df.filter(F.abs(F.col("demand_zscore")) >= 2.5).select(
        "period",
        "respondent",
        "respondent_name",
        F.lit("demand_anomaly").alias("alert_type"),
        F.when(F.abs(F.col("demand_zscore")) >= 3.5, F.lit("high"))
        .otherwise(F.lit("medium"))
        .alias("severity"),
        F.col("demand_zscore").alias("metric_value"),
        F.lit(2.5).alias("threshold_value"),
        F.concat(
            F.lit("Demand z-score reached "), F.round(F.col("demand_zscore"), 2)
        ).alias("message"),
    )

    alerts_df = (
        forecast_alerts.unionByName(ramp_alerts)
        .unionByName(coverage_alerts)
        .unionByName(demand_alerts)
        .withColumn("updated_at", F.current_timestamp())
    )
    if alerts_df.limit(1).count() > 0:
        assert_unique_keys(
            ...
            "platinum.grid_operations_alert_hourly_stage",
        )
...
def main() -> None:
    ...
    if not has_partitioned_parquet_input(
        spark, args.region_fact_path
    ...
    status_df.write.mode("overwrite").jdbc(
        jdbc_url, args.ops_stage_table, properties=jdbc_props
    )
    alerts_df.write.mode("overwrite").jdbc(
        jdbc_url, args.alerts_stage_table, properties=jdbc_props
    )
    log_job_complete(
        logger,
        "platinum_grid_operations_hourly",
