...
from common.io import has_partitioned_parquet_input, path_exists, read_partitioned_parquet
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Build resource planning daily serving tables.")
    ...
    parser.add_argument("--planning-stage-table", default="platinum.resource_planning_daily_stage")
...
def build_resource_planning_daily(region_df: DataFrame, fuel_df: DataFrame, fuel_dim_df: DataFrame) -> DataFrame:
    ...
    demand_daily_df = (
        ...
        .agg(
            ...
            F.avg(F.abs(F.col("forecast_error_pct"))).alias("avg_abs_forecast_error_pct"),
        )
    )

    fuel_with_emissions_df = fuel_df.join(
        fuel_dim_df.select("fueltype", F.col("emissions_factor_kg_per_mwh").alias("emissions_factor")),
        ["fueltype"],
        "left",
    ).withColumn("emissions_factor", F.coalesce(F.col("emissions_factor"), F.lit(0.0)))

    fuel_daily_df = (
        fuel_with_emissions_df.groupBy("date", "respondent")
        .agg(
            ...
            F.sum(F.col("generation_mwh") * F.col("emissions_factor")).alias("weighted_emissions"),
        )
    )
    ...
    fuel_share_df = (
        ...
        .join(
            fuel_df.groupBy("date", "respondent").agg(F.sum("generation_mwh").alias("total_generation_mwh")),
            ["date", "respondent"],
            "left",
        )
        .withColumn(
            "fuel_share",
            F.when(F.col("total_generation_mwh") > 0, F.col("fuel_generation_mwh") / F.col("total_generation_mwh")).otherwise(F.lit(0.0)),
        )
        .groupBy("date", "respondent")
        .agg((F.lit(1.0) - F.sum(F.pow(F.col("fuel_share"), F.lit(2.0)))).alias("fuel_diversity_index"))
    )

    peak_window = Window.partitionBy("respondent", F.to_date("period")).orderBy(F.col("actual_demand_mwh").desc(), F.col("period").asc())
    peak_hours_df = (
        region_df.filter(F.col("actual_demand_mwh").isNotNull())
    ...
    peak_fuel_df = (
        peak_hours_df.join(
            fuel_hourly_agg_df,
            (peak_hours_df.peak_period == fuel_hourly_agg_df.period) & (peak_hours_df.respondent == fuel_hourly_agg_df.respondent),
            "left",
        )
        .select(
            peak_hours_df.date,
            peak_hours_df.respondent,
            F.when(F.col("total_generation_mwh") > 0, (F.col("gas_generation_mwh") / F.col("total_generation_mwh")) * 100.0).alias("peak_hour_gas_share_pct"),
        )
    )
    ...
    planning_df = (
        ...
        .withColumn("respondent_name", F.coalesce(F.col("respondent_name"), F.col("fuel_respondent_name"), F.col("respondent")))
        ...
        .withColumn(
            ...
            F.when(F.col("total_generation_mwh") > 0, F.col("weighted_emissions") / F.col("total_generation_mwh")),
        )
        .withColumn(
            "clean_coverage_ratio",
            F.when((F.col("renewable_generation_mwh").isNotNull()) & (F.col("daily_demand_mwh") > 0), F.col("renewable_generation_mwh") / F.col("daily_demand_mwh")),
        )
    ...
    assert_no_nulls(planning_df, ["date", "respondent", "respondent_name", "daily_demand_mwh"], "platinum.resource_planning_daily_stage")
    assert_unique_keys(planning_df, ["date", "respondent"], "platinum.resource_planning_daily_stage")
    assert_non_negative(
        planning_df,
        ["daily_demand_mwh", "peak_hourly_demand_mwh", "avg_abs_forecast_error_pct", "carbon_intensity_kg_per_mwh", "fuel_diversity_index", "clean_coverage_ratio"],
        "platinum.resource_planning_daily_stage",
    )
    assert_value_bounds(planning_df, "renewable_share_pct", min_value=0.0, max_value=100.0, label="platinum.resource_planning_daily_stage")
    assert_value_bounds(planning_df, "fossil_share_pct", min_value=0.0, max_value=100.0, label="platinum.resource_planning_daily_stage")
    assert_value_bounds(planning_df, "gas_share_pct", min_value=0.0, max_value=100.0, label="platinum.resource_planning_daily_stage")
    assert_value_bounds(planning_df, "fuel_diversity_index", min_value=0.0, max_value=1.0, label="platinum.resource_planning_daily_stage")
    assert_value_bounds(planning_df, "peak_hour_gas_share_pct", min_value=0.0, max_value=100.0, label="platinum.resource_planning_daily_stage")
    return planning_df
...
def main() -> None:
    ...
    if not has_partitioned_parquet_input(spark, args.region_fact_path) or not has_partitioned_parquet_input(spark, args.fuel_fact_path) or not path_exists(spark, args.fuel_dim_path):
        return
    ...
    planning_df.write.mode("overwrite").jdbc(jdbc_url, args.planning_stage_table, properties=jdbc_props)
    log_job_complete(
        logger,
        "platinum_resource_planning_daily",
        stage_table=args.planning_stage_table,
