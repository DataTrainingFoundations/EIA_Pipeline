...
class FakeRead:
    ...
    def parquet(self, path: str):  # noqa: ANN201
        ...
        if self.calls <= self.failures_before_success:
            raise RuntimeError(f"java.io.FileNotFoundException: No such file or directory: {path}")
        self.path = path
        return {"path": path, "options": list(self.options)}


class FakeSpark:
    def __init__(self, *, exists: bool, glob_status, failures_before_success: int = 0) -> None:  # noqa: ANN001
        fs = SimpleNamespace(
            exists=lambda path: exists and path.path_str == "/data/input",
            globStatus=lambda path: glob_status if path.path_str.endswith("*.parquet") else None,
        )
...
def test_quality_helpers_raise_for_bad_data(spark_session) -> None:
    ...
    assert_value_bounds(valid_df, "value", min_value=0.0, max_value=20.0, label="valid_df")
...
def test_io_helpers_and_window_filtering_cover_path_and_read_branches(spark_session) -> None:
    ...
    assert spark_io.glob_has_matches(fake_spark, "/data/input/event_date=*/*.parquet") is True
    assert spark_io.partitioned_parquet_glob("s3a://silver/test") == "s3a://silver/test/event_date=*/*.parquet"
    ...
    assert spark_io.read_parquet_if_exists(fake_spark, "/data/input")["path"] == "/data/input"
    assert spark_io.read_parquet_if_exists(FakeSpark(exists=False, glob_status=[]), "/missing") is None
    ...
    filtered = filter_time_window(timestamp_df, "period", "2026-03-10T01:00:00+00:00", "2026-03-10T03:00:00+00:00")

    assert [row["value"] for row in filtered.orderBy("value").collect()] == [2, 3]


def test_read_helpers_retry_transient_missing_file_errors(monkeypatch) -> None:  # noqa: ANN001
    monkeypatch.setattr(spark_io.time, "sleep", lambda _: None)
    fake_spark = FakeSpark(exists=True, glob_status=[object()], failures_before_success=2)

    partitioned = spark_io.read_partitioned_parquet(fake_spark, "/data/input", retries=2, retry_delay_seconds=0.01)
    existing = spark_io.read_parquet_if_exists(fake_spark, "/data/input", retries=2, retry_delay_seconds=0.01)
