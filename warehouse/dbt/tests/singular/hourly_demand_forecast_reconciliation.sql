...
with silver_forecast as (
    select
        period_ts,
        respondent as ba_code,
        round(sum(value_gwh), 4) as expected_forecast_gwh,
        partition_date
...
gold_demand as (
    select
        period_ts,
        ba_code,
        round(forecast_gwh, 4) as actual_forecast_gwh,
        partition_date
...
select
    ...
    s.expected_forecast_gwh,
    g.actual_forecast_gwh
from silver_forecast s
full outer join gold_demand g
    on s.period_ts = g.period_ts
   and s.ba_code = g.ba_code
   and s.partition_date = g.partition_date
where coalesce(s.expected_forecast_gwh, -1) <> coalesce(g.actual_forecast_gwh, -1)
